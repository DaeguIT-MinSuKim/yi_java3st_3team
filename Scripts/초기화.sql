-- 도서관 프로젝트
DROP SCHEMA IF EXISTS library_team;

-- 도서관 프로젝트
CREATE SCHEMA library_team;

-- 우편번호
CREATE TABLE library_team.zip_code (
...
-- 우편번호
ALTER TABLE library_team.zip_code
...
-- 직책
CREATE TABLE library_team.title (
...
-- 직책
ALTER TABLE library_team.title
...
-- 출납관리
CREATE TABLE library_team.lending (
	...
	rturn_psm_cdt  INTEGER     NOT NULL COMMENT '반납연기여부', -- 반납연기여부
	rturn_date     DATE        NULL     COMMENT '반납일', -- 반납일
	overdue_cdt    INTEGER     NOT NULL COMMENT '연체여부', -- 연체여부
	overdue_date   INTEGER     NULL     COMMENT '연체일' -- 연체일
)
COMMENT '출납관리';

-- 출납관리
ALTER TABLE library_team.lending
	ADD CONSTRAINT PK_lending -- 출납관리 기본키
		PRIMARY KEY (
			lend_rturn_no -- 대여반납번호
		);

-- 중분류
CREATE TABLE library_team.middle_classification (
...
-- 중분류
ALTER TABLE library_team.middle_classification
...
-- 도서
CREATE TABLE library_team.book (
	...
	lend_psb_cdt INTEGER     NOT NULL COMMENT '대여가능여부', -- 대여가능여부
	...
	dsuse_cdt    INTEGER     NULL     COMMENT '폐기여부' -- 폐기여부
)
COMMENT '도서';

-- 도서
ALTER TABLE library_team.book
...
-- 추천도서
CREATE TABLE library_team.recommendation (
...
-- 추천도서
ALTER TABLE library_team.recommendation
...
-- 사서
CREATE TABLE library_team.librarian (
	...
	work_cdt    INTEGER      NOT NULL COMMENT '근무여부' -- 근무여부
)
COMMENT '사서';

-- 사서
ALTER TABLE library_team.librarian
...
-- 신청도서
CREATE TABLE library_team.request_book (
	...
	wh_cdt            INTEGER     NULL     COMMENT '입고여부' -- 입고여부
)
COMMENT '신청도서';

-- 신청도서
ALTER TABLE library_team.request_book
...
-- 대분류
CREATE TABLE library_team.large_classification (
	lclas_no   INTEGER     NOT NULL COMMENT '대분류번호', -- 대분류번호
...
-- 대분류
ALTER TABLE library_team.large_classification
...
-- 회원
CREATE TABLE library_team.member (
	mber_id       VARCHAR(30)  NOT NULL COMMENT '회원ID', -- 회원ID
	...
	wdr_cdt       INTEGER      NULL     COMMENT '탈퇴여부' -- 탈퇴여부
)
COMMENT '회원';

-- 회원
ALTER TABLE library_team.member
...
-- 출판사
CREATE TABLE library_team.publishing_company (
...
-- 출판사
ALTER TABLE library_team.publishing_company
...
-- 회원등급
CREATE TABLE library_team.grade (
	grade_no        INTEGER     NOT NULL COMMENT '등급번호', -- 등급번호
	grad_name       VARCHAR(50) NOT NULL COMMENT '등급이름', -- 등급이름
	book_le_psb_cdt INTEGER     NOT NULL COMMENT '도서대여가능여부', -- 도서대여가능여부
	book_le_cnt     INTEGER     NOT NULL COMMENT '도서대여권수', -- 도서대여권수
	rt_psm_dt_cnt   INTEGER     NOT NULL COMMENT '반납연기일수' -- 반납연기일수
)
COMMENT '회원등급';

-- 회원등급
ALTER TABLE library_team.grade
...
-- 출납관리
ALTER TABLE library_team.lending
	ADD CONSTRAINT FK_book_TO_lending -- 도서 -> 출납관리
		FOREIGN KEY (
			book_cd -- 도서코드
		)
		REFERENCES library_team.book ( -- 도서
			book_code -- 도서코드
		),
...
-- 출납관리
ALTER TABLE library_team.lending
	ADD CONSTRAINT FK_member_TO_lending -- 회원 -> 출납관리
		FOREIGN KEY (
			mber_id -- 회원ID
		)
		REFERENCES library_team.member ( -- 회원
			mber_id -- 회원ID
		);
