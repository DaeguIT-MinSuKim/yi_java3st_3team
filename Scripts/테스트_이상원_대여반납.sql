...
begin
	...
	start transaction;
		...
		select count(*) into cnt 
			from lending
			where overdue_cdt = 1 and mber_id = 'xodnjs1218@naver.com' and rturn_date is null and DATEDIFF(rturn_date, rturn_due_date ) > 0;
		-- 반납대여 테이블
		/* 반납대여 테이블에서 반납일의 null값을 반납날짜로 업데이트*/
		update lending 
			set rturn_date = date_format(curdate(), "%Y-%m-%d")
...
update lending
	set overdue_cdt = 1
	where DATEDIFF(curdate() , rturn_due_date ) > 0 and mber_id = 'xodnjs1218@naver.com' and book_cd = '0601.001-1';

select count(*) 
	from lending
	where overdue_cdt = 1  and rturn_date is null and mber_id = 'xodnjs1218@naver.com' and book_cd = '0601.001-1' and DATEDIFF(curdate() , rturn_due_date ) > 0;
